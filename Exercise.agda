...
lem11' : (x y c : Nat) -> x == y -> (x + c) == (y + c)
lem11' x y c eq = trans (plusCommutes x c) (trans (constAddL c x y eq) (plusCommutes c y))

lem1 : (n m k : Nat)
     -> ((n * (m + k)) == ((n * m) + (n * k)))
     -> (((m + k) + (n * (m + k))) == ((m + k) + ((n * m) + (n * k))))
lem1 n m k ih = constAddL (m + k) (n * (m + k)) ((n * m) + (n * k)) ih
...
addLemma x y z w =
 ...
 ≡⟨ constAddL x (y + (z + w)) ((y + z) + w) (associativity y z w) ⟩ (
  (x + ((y + z) + w))
 ≡⟨ constAddL x ((y + z) + w) ((z + y) + w) (
      lem11' (y + z) (z + y) w (plusCommutes y z)
     )⟩ (
  (x + ((z + y) + w))
...
subListNil : {a : Set} {xs : List a} -> SubList Nil xs
...
SubListLem : {a : Set} {xs ys : List a} -> (x : a) -> SubList (Cons x ys) (Cons x xs) -> SubList ys xs
SubListLem x (Keep h) = h
SubListLem x (Drop h) = SubListTrans (Drop SubListRefl) h

SubListLem' : {a : Set} {xs ys : List a} -> (x y : a)
           -> SubList (Cons x (Cons y ys)) xs -> SubList (Cons y ys) xs
SubListLem' x y (Keep h) = Drop h
SubListLem' x y (Drop h) = Drop (SubListLem' x y h)

SubListContra : {a : Set} {xs : List a} -> (x : a) -> SubList (Cons x xs) xs -> Empty
...
correctness {n} {st} (Add e e') =
  sym ((stackAdd (exec (compile e >> compile e') st))
